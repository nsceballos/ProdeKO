...
import { resultWinnerSide } from './result'
...
// En las eliminatorias un empate en los 90'/alargue se resuelve por penales:
// resultWinnerSide usa la definición por penales (formato "1-1 (P 4-3)") para
// determinar quién avanza. Sin penales cargados, un empate no define ganador.
function getWinner(matchId, resolved, resultsMap) {
  const side = resultWinnerSide(resultsMap[matchId])
  if (!side) return null
